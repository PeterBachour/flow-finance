...
def test_v57_release_surfaces_are_synchronized():
    ...
    assert "const VERSION='6.0.0';" in runtime
    assert "const VERSION='6.0.0';" in service_worker
    assert '?v=6.0.0' in index
    assert '?v=5.5.0' not in index
    assert '?v=6.0.0' in service_worker
    assert '?v=5.5.0' not in service_worker
    assert '?v=6.0.0' in manifest
...
def test_v57_release_is_first_in_exposed_changelog():
    data = changelog()
    assert data['releases'][0]['version'] == '6.0.0'
    assert data['releases'][0]['title'] == 'Flow V5.7 — Data Quality Action Center'
