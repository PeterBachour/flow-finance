...
# Strictly deterministic descriptors only. Ambiguous merchant names, personal
# transfers, travel-like purchases and generic food/retail names are excluded.
# Keep this list aligned with the auto_classifiable tier from
# audit_repeated_merchant_classification, but only use categories that are part
# of Flow's operational category model.
RULES = (
    ('PRLV SEPA DIRECTION GENERALE DES FINANCES PUBLIQUE', 'Impôts', 'explicit French tax authority debit'),
    ('TCL 69 LYO', 'Transport', 'explicit Lyon public transport descriptor'),
    ('VELIB METROPOLE', 'Transport', 'explicit Vélib merchant'),
    ('PAYBYPHONE', 'Transport', 'parking payment service'),
    ('RIE PEREIRE', 'Restaurants', 'company restaurant descriptor'),
    ('LOUBNANI', 'Restaurants', 'restaurant merchant identity'),
    ('KR POKE', 'Restaurants', 'restaurant merchant identity'),
    ('BRASSERIE', 'Restaurants', 'brasserie merchant identity'),
    ('BEERHOUSE', 'Restaurants', 'food and beverage merchant identity'),
    ('FIVE GUYS', 'Restaurants', 'known restaurant chain'),
    ('POKAWA', 'Restaurants', 'known restaurant chain'),
    ('NEWREST WAGONS', 'Restaurants', 'on-board catering merchant'),
    ('LEROY MERLIN', 'Shopping', 'home improvement retailer'),
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Dry-run or apply deterministic category fixes to Flow with an SQLite backup before mutation.'
    )
    ...
    try:
        ...
        by_rule: dict[str, dict[str, int]] = {
            pattern: {'rows': 0, 'amount': 0} for pattern, _, _ in RULES
        }
        for row in rows:
            text = normalize(row['label'])
            match = next(
                ((pattern, category, reason) for pattern, category, reason in RULES if pattern in text),
                None,
            )
            if not match:
                continue
        ...
        for pattern, category, reason in RULES:
            ...
            print(
                f'rule={pattern} category={category} rows={stats["rows"]} '
                f'amount={euros(stats["amount"])} reason={reason}'
            )
        ...
        if args.apply and matches:
            ...
            backup_path = backup_dir / f'{db_path.stem}.pre-v422-categories-{stamp}.db'
        ...
        print('rule=ambiguous_merchants_are_never_modified')
        print('rule=personal_transfers_are_never_modified')
