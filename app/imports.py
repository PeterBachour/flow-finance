...
def import_review_groups(conn, limit: int = 100) -> dict:
    total_pending = conn.execute("SELECT COUNT(*) total FROM transaction_import_meta WHERE review_status='needs_review'").fetchone()['total']
    pending = conn.execute("SELECT m.normalized_label,t.amount_cents,t.booking_date,t.id,t.label FROM transaction_import_meta m JOIN transactions t ON t.id=m.transaction_id WHERE m.review_status='needs_review' AND TRIM(COALESCE(m.normalized_label,''))<>''").fetchall()
    buckets = {}
    for row in pending:
        label = row['normalized_label']
        key = next((candidate for candidate in buckets if label.startswith(candidate + ' ')), label)
        buckets.setdefault(key, []).append(row)
    groups = []
    for label, items in buckets.items():
        if len(items) < 2:
            continue
        groups.append({'normalized_label': label, 'occurrence_count': len(items), 'net_amount_cents': sum(int(i['amount_cents']) for i in items), 'volume_cents': sum(abs(int(i['amount_cents'])) for i in items), 'first_seen': min(i['booking_date'] for i in items), 'last_seen': max(i['booking_date'] for i in items), 'sample_transaction_id': min(int(i['id']) for i in items), 'sample_label': min(i['label'] for i in items)})
    groups.sort(key=lambda item: (-item['occurrence_count'], -item['volume_cents'], item['normalized_label']))
    groups = groups[:limit]
    return {'total_pending': int(total_pending), 'grouped_pending': sum(int(row['occurrence_count']) for row in groups), 'groups': groups}
